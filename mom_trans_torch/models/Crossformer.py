...
class CrossformerBaseline(DeepMomentumNetwork):
    def __init__(self,
                 input_dim, num_tickers, hidden_dim, dropout,
        ...
        self.crossformer = Crossformer(
            dims=[hidden_dim],
            cross_size=[96],   # tune per your data
            num_heads=[4],
            ...
        )
    ...
    def forward_candidate_arch(self, x, tickers, pos_encoding_batch=None, **kwargs):
        ...
        if self.masking:
            ...
            mask = None
        x = self.grn(x)
        out = self.crossformer(x, attn_mask=mask)  # dims: [B, T, hidden_dim]
